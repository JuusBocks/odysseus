...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    "bash": "Run shell commands on the server. Install packages, git operations, builds, system info, process management, and local automation scripts such as Playwright/browser smoke tests. Prefer a dedicated tool whenever one fits the job (file read/write/edit, search, listing); use bash only for what no dedicated tool covers. Do not use for web lookup/search; use web_search or web_fetch when web tools are available.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"theme", "color scheme", "colors of the ui", "make it dark",
            ...
            {"ui_control"},
        # Local browser automation / e2e test intent.
        frozenset({"playwright", "browser automation", "automate browser",
                   "automate the browser", "e2e", "end-to-end", "smoke test",
                   "screenshot", "clipboard", "dropdown", "locator"}):
            {"bash", "python", "read_file", "grep", "ls"},
