...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "ask_teacher": "- ```ask_teacher``` — Escalate to the configured teacher model. Line 1 = model name or 'auto', rest = the question. Before calling, summarize the need and replace private values with placeholders; Odysseus also redacts common secrets before sending externally. Use when stuck or when a stronger model should do hard planning/review. REQUIRED when the user explicitly asks you to use/ask the teacher, use a teacher/student flow, or show what was redacted.",
...
_MCP_KEYWORDS = frozenset(["mcp", "browse", "browser", "playwright", "website", "calendar", "event", "email",
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if "notes_calendar_tasks" not in domains and has(r"\bwrite\b"):
        domains.add("documents")
    if has(r"\b(search|web|google|look up|latest|news|current|weather|forecast|stock price|price of|website|url|https?://|www\.|browse|browser)\b"):
        domains.add("web")
    if has(r"\b(playwright|browser automation|automate (?:the )?browser|web actions?|screenshot|clipboard|dropdown|locator|e2e|end-to-end)\b"):
        domains.add("files")
        domains.add("web")
...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, block in enumerate(tool_blocks):
            ...
            # Forward structured teacher-exchange metadata for the visual
            # student/privacy/teacher card. The plain response remains in
            # output_text for the student model's next round.
            if "teacher_exchange" in result:
                tool_output_data["teacher_exchange"] = result["teacher_exchange"]
            yield f'data: {json.dumps(tool_output_data)}\n\n'
